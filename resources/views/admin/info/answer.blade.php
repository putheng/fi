...
<form action="{{ route('admin.question.info.answer.create', $info) }}" method="post" class="form-horizontal">
    {{ csrf_field() }}
    @foreach($info->answers as $key => $item)
        <div class="form-group {{ $errors->has('answer.'. $key) ? ' has-error' : '' }}">
            ...
            <div class="col-md-4">
                ...
                <input class="form-control font-sr" 
                    ...
                    >
                    @if($errors->has('answer_en.'. $key))
                        <span class="text-danger">{{ $errors->first('answer_en.'. $key) }}</span>
                    @endif
                </div>

            <div class="col-md-2">
                <a href="{{ route('admin.question.info.answer.delete', $item) }}" class="btn btn-link">Delete</a>
            </div>
        </div>
        <hr>
    @endforeach
